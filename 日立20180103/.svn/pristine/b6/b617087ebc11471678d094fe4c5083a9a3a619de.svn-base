...
namespace PM2.Service.Code030
{
    public class szrl121Server : Iszrl121Server
    {
        ...
        private IEFRepository<szrl129> _szrl129Repository;//验收处理变更
        private IEFRepository<szrl130> _szrl130Repository;//验收处理取消
        ...
        public IOperateResult IsCanChanges(HttpCollection vParams)
        {
            try
            {
                ...
                using (IDbContextScope scope = this._scopeFactory.CreateScope())
                {
                    //这样要读的是124,129表，看最新的版本是否已审核，不然不允许做验收操作
                    var _129lst = this._szrl129Repository.GetModels(y => y.rl12902 == Caigouhetongid);
                    var _129model = (from a in this._szrl129Repository.GetModels(y => y.rl12902 == Caigouhetongid)
                                     where a.rl12913 == _129lst.Max(c => c.rl12913)
                                     select a).ToList().FirstOrDefault();
                    //除了读变更表，还要读取消表，看最新的版本是否已审核，不然不允许做验收操作
                    var _130lst = this._szrl130Repository.GetModels(x => x.rl13002 == Caigouhetongid);
                    var _130model = (from a in this._szrl130Repository.GetModels(y => y.rl13002 == Caigouhetongid)
                    ...
                    if (_129model != null)
                    {
                        if (_129model.rl12904 == 1 && _129model.rl12909 == 1)
                        {
                            IsTrue = true; 
                        }
                        else
                    ...
                    if (_130model != null)
                    {
                        if (_130model.rl13004 == 1 && _130model.rl13009 == 1)
                        {
                            IsTrue = true; 
                        }
                        else
